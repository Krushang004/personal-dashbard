    ...
    return (
        <div className="card p-5 flex flex-col gap-4">
            <div className="flex items-center gap-2">
                <div style={{ background: '#f8717120', borderRadius: 8, padding: '6px' }}>
                ...
                {MOODS.map(m => (
                    <button key={m.key} onClick={() => select(m.key)}
                        style={{
                            ...
                            background: today.mood === m.key ? m.color + '20' : 'rgba(255,255,255,0.03)',
                        ...
                        <span style={{ fontSize: 10, color: today.mood === m.key ? m.color : '#8b8a96', fontWeight: 500 }}>{m.label}</span>
                    </button>
                ))}
            </div>
            ...
            <div>
                    ...
                    {weekData.map((d, i) => {
                        ...
                        return (
                            <div key={i} title={mood?.label || 'No entry'}
                                ...
                                {mood ? mood.emoji : <span style={{ color: '#2a2a36', fontSize: 10 }}>•</span>}
                            </div>
                        );
                    })}
                </div>
                <div style={{ display: 'flex', gap: 6, marginTop: 2 }}>
                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
                        <div key={i} style={{ flex: 1, textAlign: 'center', fontSize: 9, color: '#4b4a56' }}>{d}</div>
                    ))}
                </div>
            </div>
        </div>
